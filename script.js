                   ...
                   const counts = labels.reduce((acc, label) => {
                      ...
                      for (const sectionName2 in results) {
                          for (const questionStatement2 in results[sectionName2]) {
                              const question2 = surveyQuestions
                                  .find(section => section.section === sectionName2)
                                  .questions.find(question => question.statement === questionStatement2);

                              if (question2 && question2.identifier === question.identifier) {

                                 const responseIndex = results[sectionName2][questionStatement2];
                                    const responseLabel = question.options[responseIndex-1];
                                 if (responseLabel)
                                  counts[responseLabel]++;
                    ...
                    canvas.id = `chart-${sectionName}-${questionStatement.replace(/\s+/g, '-')}`;
